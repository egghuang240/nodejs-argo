...
// 全局常量 - 【改动 1】直接使用镜像里的 xray 路径
const npmName = generateRandomName();
const webName = generateRandomName();
const botName = generateRandomName();
const phpName = generateRandomName();
let npmPath = '/tmp/npm_dummy';             // 哪吒占位，不启动
let phpPath = '/tmp/php_dummy';             // 哪吒占位，不启动
let webPath = '/usr/local/bin/xray';        // ★ 改动：直接用镜像里的 xray
let botPath = '/tmp/bot_dummy';             // cloudflared 占位，不启动
...
// ★ 改动 2：替换原 downloadFilesAndRun 函数
// 只启动 xray，不下载任何东西，不启动 cloudflared 和哪吒
async function downloadFilesAndRun() {  
  console.log('Skipping download - using pre-installed binaries in image');
  ...
  if (fs.existsSync(webPath)) {
    ...
    console.log(`xray not found at ${webPath}`);
  }
}

//根据系统架构返回对应的url (保留原函数但不使用)
